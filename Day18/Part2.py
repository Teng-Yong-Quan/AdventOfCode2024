...
def part2(wall_limit):
	end_lst = []
	walls = original_walls[:wall_limit]
	pq = [(start,0)]
	shortest_visited = []
	while pq:
		new_lst = []
...
low_wall_limit = 1024
high_wall_limit = len(original_walls)
while low_wall_limit + 1 < high_wall_limit:
	mid_wall_limit = (low_wall_limit + high_wall_limit) // 2
	end_lst = part2(mid_wall_limit)
	#print(f"low_wall_limit: {low_wall_limit}, high_wall_limit: {high_wall_limit}, mid_wall_limit: {mid_wall_limit}, end_lst: {end_lst}")
	if end_lst:
		low_wall_limit = mid_wall_limit
	else:
		high_wall_limit = mid_wall_limit
#print(high_wall_limit)
ans = original_walls[high_wall_limit - 1]
print(ans)
input_data.close()